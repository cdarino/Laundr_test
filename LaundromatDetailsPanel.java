...
public class LaundromatDetailsPanel extends JPanel {
    ...
    // New fields for the restructured layout
    private roundedPanel descriptionPanel;
    private roundedPanel servicesWrapperPanel;
    ...
    public LaundromatDetailsPanel() {
        ...
        // ADDED: Wrapper panel with top margin for content, and negative horizontal margins to align with header
        JPanel contentWrapper = new JPanel(new BorderLayout());
        contentWrapper.setOpaque(false);
        contentWrapper.setBorder(new EmptyBorder(40, -20, 0, -20)); // CHANGED: 40px top, -20px left/right to compensate for main panel border

        // === NEW LAYOUT: OUTER PANEL WITH LEFT AND RIGHT SECTIONS ===
        JPanel outerPanel = new JPanel(new GridBagLayout());
        outerPanel.setOpaque(false);
        GridBagConstraints outerGbc = new GridBagConstraints();
        outerGbc.gridy = 0;
        outerGbc.fill = GridBagConstraints.BOTH;
        outerGbc.weighty = 1.0; // Allow vertical expansion to fill space
        ...
        // Description Panel (top of left) - LARGER
        descriptionPanel = new roundedPanel(18);
        descriptionPanel.setLayout(new BorderLayout());
        descriptionPanel.setBackground(UIManager.getColor("background"));
        ...
        leftGbc.gridy = 0;
        leftGbc.weighty = 0.6; // Description panel is LARGER
        outerLeftPanel.add(descriptionPanel, leftGbc);

        // Gap between description and services
        leftGbc.gridy = 1;
        leftGbc.weighty = 0;
        leftGbc.insets = new Insets(20, 0, 0, 0);
        outerLeftPanel.add(Box.createVerticalStrut(20), leftGbc);

        // Services Panel (bottom of left) - SMALLER
        servicesWrapperPanel = new roundedPanel(18);
        servicesWrapperPanel.setLayout(new BorderLayout());
        servicesWrapperPanel.setBackground(UIManager.getColor("background"));
        servicesWrapperPanel.setBorder(BorderFactory.createCompoundBorder(
                new roundedBorder(18, UIManager.getColor("listBorder"), 2),
                new EmptyBorder(20, 20, 20, 20)
        ));
        ...
        servicesWrapperPanel.add(servicesPanel, BorderLayout.CENTER);

        leftGbc.gridy = 2;
        leftGbc.weighty = 0.4; // Services panel is SMALLER
        leftGbc.insets = new Insets(0, 0, 0, 0);
        outerLeftPanel.add(servicesWrapperPanel, leftGbc);

        // Add outer left panel to main outer panel
        outerGbc.gridx = 0;
        outerGbc.weightx = 0.6; // Left panel is wider
        outerGbc.insets = new Insets(0, 0, 0, 0);
        outerPanel.add(outerLeftPanel, outerGbc);

        // Gap between left and right outer panels
        outerGbc.gridx = 1;
        outerGbc.weightx = 0;
        outerGbc.weighty = 0;
        outerGbc.fill = GridBagConstraints.NONE;
        outerGbc.insets = new Insets(0, 12, 0, 12);
        outerPanel.add(Box.createHorizontalStrut(12), outerGbc);

        // === OUTER RIGHT PANEL ===
        JPanel outerRightPanel = new JPanel(new GridBagLayout());
        ...
        // Reviews Panel (top of right)
        reviewsWrapperPanel = new roundedPanel(18);
        reviewsWrapperPanel.setLayout(new BorderLayout());
        reviewsWrapperPanel.setBackground(UIManager.getColor("background"));
        ...
        revScroll.setOpaque(false);
        revScroll.getViewport().setOpaque(false);
        reviewsWrapperPanel.add(revScroll, BorderLayout.CENTER);

        rightGbc.gridy = 0;
        rightGbc.weighty = 0.7; // Reviews panel is taller
        outerRightPanel.add(reviewsWrapperPanel, rightGbc);

        // Gap between reviews and button
        rightGbc.gridy = 1;
        rightGbc.weighty = 0;
        rightGbc.insets = new Insets(20, 0, 0, 0);
        outerRightPanel.add(Box.createVerticalStrut(20), rightGbc);

        // Pickup Button Panel (bottom of right) - NO BORDER
        pickupButtonPanel = new JPanel(new BorderLayout());
        pickupButtonPanel.setOpaque(false);
        ...
        pickupButtonPanel.add(buttonWrapper, BorderLayout.CENTER);

        rightGbc.gridy = 2;
        rightGbc.weighty = 0.3; // Button panel takes less vertical space
        rightGbc.insets = new Insets(0, 0, 0, 0);
        outerRightPanel.add(pickupButtonPanel, rightGbc);

        // Add outer right panel to main outer panel
        outerGbc.gridx = 2;
        outerGbc.weightx = 0.4; // Right panel is narrower
        outerGbc.weighty = 1.0;
        outerGbc.fill = GridBagConstraints.BOTH;
        outerGbc.insets = new Insets(0, 0, 0, 0);
        outerPanel.add(outerRightPanel, outerGbc);

        contentWrapper.add(outerPanel, BorderLayout.CENTER);
    ...
    public void setLaundromat(LaundromatData data) {
        if (placeholderWrapper.getParent() != null) {
            ...
            // Re-add the outer panel structure
            JPanel outerPanel = new JPanel(new GridBagLayout());
            outerPanel.setOpaque(false);
            GridBagConstraints outerGbc = new GridBagConstraints();
            outerGbc.gridy = 0;
            ...
            leftGbc.gridy = 0;
            leftGbc.weighty = 0.6; // Description is larger
            leftGbc.insets = new Insets(0, 0, 0, 0);
            outerLeftPanel.add(descriptionPanel, leftGbc);
            ...
            leftGbc.gridy = 2;
            leftGbc.weighty = 0.4; // Services is smaller
            leftGbc.insets = new Insets(0, 0, 0, 0);
            outerLeftPanel.add(servicesWrapperPanel, leftGbc);

            outerGbc.gridx = 0;
            outerGbc.weightx = 0.6;
            outerGbc.insets = new Insets(0, 0, 0, 0);
            outerPanel.add(outerLeftPanel, outerGbc);
    ...
    public void updateUI() {
        ...
        // Update rounded panel backgrounds
        if (descriptionPanel != null) {
            descriptionPanel.setBackground(UIManager.getColor("background"));
        }
        if (servicesWrapperPanel != null) {
            servicesWrapperPanel.setBackground(UIManager.getColor("background"));
        }
        if (reviewsWrapperPanel != null) {
            reviewsWrapperPanel.setBackground(UIManager.getColor("background"));
        }
